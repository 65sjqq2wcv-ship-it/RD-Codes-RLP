class RettungsdienstApp {
  constructor() {
    ...
    this.currentAppVersion = "1.10"; // Version synchron mit sw.js halten
  }
  ...
  async loadCodes() {
    try {
      ...
      this.showMessage(
        "Fehler beim Laden der Einsatzcodes. Fallback-Daten werden verwendet.",
        "error",
      );
  ...
  async performUpdate() {
    try {
      ...
      this.showMessage(
        "Fehler beim Update. Bitte Seite manuell neu laden.",
        "error",
      );
    }
  }
  ...
  safeRenderCategories() {
    try {
      ...
      if (!this.codes.categories) {
        ...
        grid.innerHTML =
          '<div class="empty-state"><p>Keine Kategorien verfügbar</p></div>';
        return;
      }
  ...
  // Angepasste handleMultiSearch für Farben
      ...
      Object.keys(this.codes.categories).forEach((categoryKey) => {
        ...
        if (category.codes) {
          Object.keys(category.codes).forEach((code) => {
            const codeData = this.getCodeData(category.codes[code]);
            const description = codeData.description.toLowerCase();

            if (code === term || description.includes(termLower)) {
              const alreadyAdded = results.find((r) => r.code === code);
              if (!alreadyAdded) {
                results.push({
                  code,
  ...
  // NEU: Hilfsfunktion um Code-Daten zu extrahieren
  getCodeData(codeValue) {
    if (typeof codeValue === "string") {
      return { description: codeValue, color: null };
    } else if (typeof codeValue === "object" && codeValue !== null) {
      return {
        description: codeValue.description || "",
        color: codeValue.color || null,
      };
    }
    return { description: "", color: null };
  }

  // NEU: Kontrastreiche Textfarbe berechnen
  ...
  // ANGEPASST: showAllCodes mit Farben
  safeShowAllCodes() {
    try {
      ...
      if (!this.codes.categories) {
        ...
        container.innerHTML =
          '<div class="empty-state"><h3>Keine Codes verfügbar</h3><p>Bitte laden Sie eine gültige JSON-Datei hoch</p></div>';
        return;
      }
      ...
      allCodes.sort((a, b) => parseInt(a.code) - parseInt(b.code));
      this.safeRenderFilteredCodes(container, allCodes);
  ...
  // ANGEPASST: Kompakte 2-Zeilen-Anzeige mit Farben
  safeRenderFilteredCodes(container, codes) {
    try {
      ...
      if (!codes || codes.length === 0) {
        container.innerHTML =
          '<div class="empty-state"><h3>Keine Codes gefunden</h3><p>Versuchen Sie andere Suchbegriffe</p></div>';
        return;
      }

      const html = codes
        .map((code) => {
          const bgColor = code.color || "#FFFFFF";
          const textColor = this.getContrastColor(bgColor);
          const borderColor = code.color || "#E2E8F0";

          return `
                    <div class="code-item code-item-compact" 
                         onclick="window.app.addCodeToSearch('${code.code}')"
                         style="background-color: ${bgColor}; color: ${textColor}; border-left-color: ${borderColor};">
                        <div class="code-item-header">
                            <div class="code-item-code">${code.code}</div>
                            <span class="code-item-category">${code.categoryName}</span>
                        </div>
  ...
  // ANGEPASST: searchCodes mit Farben
  searchCodes(query) {
    const results = [];
    ...
    return results.sort((a, b) => {
      ...
      if (
        aCodeLower.startsWith(queryLower) &&
        !bCodeLower.startsWith(queryLower)
      )
        return -1;
      if (
        bCodeLower.startsWith(queryLower) &&
  ...
  // ANGEPASST: displayMultiSearchResults mit Farben
  displayMultiSearchResults(codes) {
    ...
    if (codes.length === 0) {
      ...
      if (foundCodesContainer) {
        foundCodesContainer.innerHTML =
          '<div class="empty-state"><h3>Keine Codes gefunden</h3><p>Versuchen Sie andere Suchbegriffe</p></div>';
      }
      return;
    }

    const resultsHtml = codes
      .map((code) => {
        const bgColor = code.color || "#FFFFFF";
        const textColor = this.getContrastColor(bgColor);
        const borderColor = code.color || "#E2E8F0";
        ...
        return `
                <div class="result-item result-item-compact" 
                     style="background-color: ${bgColor}; color: ${textColor}; border-left-color: ${borderColor};">
                    <div class="result-item-header">
                        <div class="result-code">${code.code}</div>
                        <span class="result-category">${code.categoryName}</span>
                    </div>
  ...
  // ANGEPASST: filterByCategory mit Farben
  filterByCategory(categoryKey) {
    ...
    categoryResults.sort((a, b) => parseInt(a.code) - parseInt(b.code));
    ...
    this.showMessage(
      `${categoryResults.length} Codes in ${category.name} gefunden`,
      "success",
    );
  }
  ...
  addCodeToSearch(code) {
    ...
    const currentCodes = currentValue
      .split(/[\s\-]+/)
      .filter((c) => c.length > 0);
    const codeExists = currentCodes.some(
  ...
  async updateConfigInfo() {
    ...
    const categoriesCount = this.codes.categories
      ? Object.keys(this.codes.categories).length
      : 0;
  ...
  resetToDefault() {
    if (
      confirm(
        "Möchten Sie wirklich zur Standard-Konfiguration zurückkehren? Alle hochgeladenen Daten gehen verloren.",
      )
    ) {
      try {
        localStorage.removeItem("customCodesData");
        ...
        this.showMessage(
          "Lösche eigene Daten und lade Standard-Konfiguration...",
          "info",
        );
        ...
        this.loadCodes()
          .then(() => {
            ...
            this.showMessage(
              "Standard-Konfiguration erfolgreich wiederhergestellt!",
              "success",
            );
          })
          .catch((error) => {
            ...
            this.showMessage(
              "Fehler beim Wiederherstellen. Seite wird neu geladen...",
              "error",
            );
        ...
        this.showMessage(
          "Fehler beim Reset. Seite wird neu geladen...",
          "error",
        );
  ...
  async handleFileUpload(file) {
    try {
      ...
      if (validation.valid) {
        ...
        this.showMessage(
          `Datei "${file.name}" erfolgreich geladen!`,
          "success",
        );
        ...
        this.showMessage(
          "JSON-Datei ist nicht gültig. Siehe Validierungsdetails.",
          "error",
        );
      }
    } catch (error) {
      ...
      this.showMessage(
        "Fehler beim Lesen der JSON-Datei: " + error.message,
        "error",
      );
  ...
  debug() {
    ...
    console.log(
      "Categories:",
      this.codes?.categories ? Object.keys(this.codes.categories) : "none",
    );
